...
#### 고려사항
- 선생님이 강의 입장 전까지 학생은 선생님 아이디 모르는 구조라 채팅을 보낼 수 없음.(선생님이 바뀔수 있기 때문?)
- Unread 메시지 처리 방안.
- 강의의 선생님이 바뀌면 이전에 대화하던 데이터는 새로운 선생님에게 공유되어야 하는지?
- 디비에 저장하는 방식과 소켓서버가 전달하는 실시간 데이터 타이밍 때문에 맞지 않을 경우.(디비 저장이 안된다면?)
- 일단 레디스에 저장하고 강의가 끝나고 해당하는 채팅의 데이터를 한꺼번에 저장하는 방식으로 고려?