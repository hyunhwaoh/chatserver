...
➡ 현재 RabbitMQ 에 STOMP 플러그인하여 사용중이라 웹소켓 서버를 수평 확장하기 어려운데,
지금 가지고 있는 RabbitMQ, Redis 를 활용하면서 Socket.IO 서버만 확장가능하게 하면 좀더 수월하게 구현이 가능함.

#### 작동 방식

![chat-server-diagram.jpg](doc/chat-server-diagram.jpg)
...
1. 클라이언트가 Socket.IO netty 서버에 연결. 강의별 개설된 룸에 조인. 이때 클라이언트 접속 목록은 Redis 에 저장하여 관리.

2. 클라이언트가 소켓을 통해 메시지를 전달하면 소켓 서버가 Redis 에 시퀀스 채번 아이디 생성, 추가하여 RabbitMQ 메지시 동기화 큐에 전달.

3. RabbitMQ 에서 Fanout 하여 모든 소켓 서버에 메시지를 브로드캐스트 하는 동시에 Storage Queue 를 쌓음.

4. API 서버들이 Storage Queue 를 구독하고 있다가 메시지를 가져와 RDB 에 채팅 메시지를 저장. </br>
  큐는 경쟁 소비자 패턴으로 작동하며 배치 서버에서 메시지를 처리.

5. 새로 들어온 클라이언트는 API 로 이전 채팅 메시지를 가져오며 동시에 소켓서버에서 강의 룸에 입장하여 최신 메시지를 이어 받게 됨.

6. 강의를 듣는 사용자가 없으면 룸은 종료되고 Redis room 정보도 삭제.
...
- 메시지 데이터의 아이디는 채팅방 내에서 고유해야 하며, 시간순서 일치 정렬가능 해야함. </br>
  - Redis 기반 전역 시퀀스 번호 (Atomic Counter)
  ID 구성: 채팅방 ID + timestamp + 증가 시퀀스 번호
